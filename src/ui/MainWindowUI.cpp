...
void MainWindowUI::OnAboutButton(wxCommandEvent &event)
{
    ...
    wxString about = wxString::Format("<h2>%s</h2><p>Version: %s </p><p>Git version: %s</p><p>Website: <a target='_blank' href='%s'>%s</a></p><hr/>",
                                      PROJECT_NAME,
                                      SD_GUI_VERSION,
                                      GIT_HASH,
                                      SD_GUI_HOMEPAGE,
                                      PROJECT_NAME);
    about.append(wxString::Format("<p>Configuration folder: %s</p><p>Config file: %s</p><p>Data folder: %s</p>",
...
void MainWindowUI::m_notebook1302OnNotebookPageChanged(wxNotebookEvent &event)
{
    ...
    if (selected == 0 || selected == 1 || selected == 2 || selected == 4)
    {
        ...
        if (selected == 2) // not working on img2img.. this is hardcoded in sd.cpp to only 1 image...
        {
...
void MainWindowUI::onContextMenu(wxDataViewEvent &event)
{
    ...
    if (source == this->m_joblist)
    {
        ...
        if (qitem->mode == QM::GenerationMode::UPSCALE)
        {
            if (qitem->images.size() > 0)
            {
                menu->Append(6, wxString::Format("Upscale again"));
            }
        }
    ...
    if (source == this->m_data_model_list)
    {
        ...
        {
            menu->Append(101, wxString::Format(_("Append to text2img prompt <lora:%s:0.5>"), modelinfo->name));
            menu->Append(102, wxString::Format(_("Append to text2img neg. prompt <lora:%s:0.5>"), modelinfo->name));
            menu->Append(103, wxString::Format(_("Append to img2img prompt <lora:%s:0.5>"), modelinfo->name));
            menu->Append(104, wxString::Format(_("Append to img2img neg. prompt <lora:%s:0.5>"), modelinfo->name));
        }
        if (modelinfo->model_type == sd_gui_utils::DirTypes::EMBEDDING)
        {
            menu->Append(111, wxString::Format(_("Append to text2img prompt %s"), modelinfo->name));
            menu->Append(112, wxString::Format(_("Append to text2img neg. prompt %s"), modelinfo->name));
            menu->Append(113, wxString::Format(_("Append to img2img prompt %s"), modelinfo->name));
            menu->Append(114, wxString::Format(_("Append to img2img neg. prompt %s"), modelinfo->name));
        }
        menu->Bind(wxEVT_COMMAND_MENU_SELECTED, &MainWindowUI::OnPopupClick, this);
        PopupMenu(menu);
        delete menu;
...
void MainWindowUI::onTxt2ImgFileDrop(wxDropFilesEvent &event)
{
    ...
    {
        wxMessageBox(wxString::Format(_("Image type not supported: %s"), path.extension().string()));
        return;
    }
    ...
    {
        wxMessageBox(wxString::Format(_("Only jpeg supported: %s"), path.extension().string()));
        return;
    }
...
void MainWindowUI::OnUpscalerFactorChange(wxSpinEvent &event)
{
    ...
    {
        this->m_static_upscaler_height->SetLabel(wxString::Format(_("%dpx"), this->currentUpscalerSourceImage->GetHeight()));
        this->m_static_upscaler_width->SetLabel(wxString::Format(_("%dpx"), this->currentUpscalerSourceImage->GetWidth()));
...
void MainWindowUI::onSavePreset(wxCommandEvent &event)
{
    ...
    if (dlg.ShowModal() == wxID_OK)
    {
        ...
        std::string presetfile = wxString::Format("%s%s%s.json",
                                                  this->cfg->presets,
...
void MainWindowUI::UpdateModelInfoDetailsFromModelList(sd_gui_utils::ModelFileInfo *modelinfo)
{
    ...
    for (auto img : modelinfo->CivitAiInfo.images)
    {
        ...
        {
            wxImage resized = sd_gui_utils::cropResizeImage(img.local_path, 256, 256, wxColour(*wxBLACK), this->cfg->thumbs_path);
            ...
            auto tooltip = wxString::Format("Resolution: %s Seed: %" PRId64 " Steps: %d", img.meta.Size, img.meta.seed, img.meta.steps);
                             ...
                             if (!img.meta.hashes.model.empty()) {
                                ...
                                if (!imgsModel.civitaiPlainJson.empty()) {
                                   ...
                                   menu->Bind(wxEVT_COMMAND_MENU_SELECTED, [this,img,modelinfo](wxCommandEvent &evt){
                                        ...
                                        break;
                                        case 10:{
                                            //wxString url = wxString::Format("https://civitai.com/images/%d",img.i);
...
{
    FreeSdCtxFunction free_sd_ctx = (FreeSdCtxFunction)this->sd_dll->GetSymbol("free_sd_ctx");
    FreeUpscalerFunction free_upscaler_ctx = (FreeUpscalerFunction)this->sd_dll->GetSymbol("free_upscaler_ctx");
    if (this->modelLoaded)
    {
    ...
    */
    if (tu >= 100 && tu < 300)
    {
        ...
        case 100:
            ...
            break;
        case 101:
            this->m_prompt->SetValue(wxString::Format("%s <lora:%s:0.5>", this->m_prompt->GetValue(), modelinfo->name));
            break;
        case 102:
            this->m_neg_prompt->SetValue(wxString::Format("%s <lora:%s:0.5>", this->m_neg_prompt->GetValue(), modelinfo->name));
            break;
        case 103:
            this->m_prompt2->SetValue(wxString::Format("%s <lora:%s:0.5>", this->m_prompt2->GetValue(), modelinfo->name));
            break;
        case 104:
            this->m_neg_prompt2->SetValue(wxString::Format("%s <lora:%s:0.5>", this->m_neg_prompt2->GetValue(), modelinfo->name));
            break;
        case 105:
            this->threads.emplace_back(new std::thread(&MainWindowUI::threadedModelInfoDownload, this, this->GetEventHandler(), modelinfo));
            break;
        case 111:
            this->m_prompt->SetValue(wxString::Format("%s %s", this->m_prompt->GetValue(), modelinfo->name));
            break;
        case 112:
            this->m_neg_prompt->SetValue(wxString::Format("%s %s", this->m_neg_prompt->GetValue(), modelinfo->name));
            break;
        case 113:
            this->m_prompt2->SetValue(wxString::Format("%s %s", this->m_prompt2->GetValue(), modelinfo->name));
            break;
        case 114:
            this->m_neg_prompt2->SetValue(wxString::Format("%s %s", this->m_neg_prompt2->GetValue(), modelinfo->name));
            break;
        case 115:
            this->threads.emplace_back(new std::thread(&MainWindowUI::threadedModelInfoDownload, this, this->GetEventHandler(), modelinfo));
            break;
...
std::string MainWindowUI::paramsToImageComment(QM::QueueItem myItem, sd_gui_utils::ModelFileInfo modelInfo)
{
    ...
    {
        comment = comment + wxString::Format("\nNegative prompt: %s", myItem.params.negative_prompt).ToStdString();
    }
    comment = comment + wxString::Format("\nSteps: %d, Seed: %" PRId64 ", Sampler: %s, CFG scale: %.1f, Size: %dx%d, Parser: sd.cpp, Model: %s, Model hash: %s, Backend: sd.cpp, App: %s, Operations: %s",
...
void MainWindowUI::StartGeneration(QM::QueueItem *myJob)
{
    ...
    case QM::GenerationMode::TXT2IMG:
        this->threads.emplace_back(new std::thread(&MainWindowUI::GenerateTxt2img, this, this->GetEventHandler(), myJob));
        break;
    case QM::GenerationMode::IMG2IMG:
        this->threads.emplace_back(new std::thread(&MainWindowUI::GenerateImg2img, this, this->GetEventHandler(), myJob));
        break;
    case QM::GenerationMode::UPSCALE:
        this->threads.emplace_back(new std::thread(&MainWindowUI::GenerateUpscale, this, this->GetEventHandler(), myJob));
        break;
    default:
        return;
        break;
...
void MainWindowUI::OnThreadMessage(wxThreadEvent &e)
{
    ...
    if (threadEvent == sd_gui_utils::ThreadEvents::QUEUE)
    {
        ...
        case QM::QueueEvents::ITEM_START: // this is just the item start, if no mode loaded, then wil trigger model load event
        {
            ...
            message = wxString::Format(_("%s is just stared to generate %d images\nModel: %s"), sd_gui_utils::modes_str[item->mode], item->params.batch_count, item->model).ToStdString();
            if (item->mode == QM::GenerationMode::UPSCALE)
            {
        ...
        case QM::QueueEvents::ITEM_FINISHED:
        {
            ...
            else
            {
                if (item->params.batch_count > 1)
                {
                    title = wxString::Format(_("%d images generation done"), item->params.batch_count).ToStdString();
                }
                else
                {
        ...
        case QM::QueueEvents::ITEM_MODEL_FAILED: // MODEL_LOAD_ERROR
            ...
            message = wxString::Format(_("The '%s' just failed to load... for more details please see the logs!"), item->model).ToStdString();
            this->ShowNotification(title, message);
            break;
        case QM::QueueEvents::ITEM_GENERATION_STARTED: // GENERATION_START
            if (item->mode == QM::GenerationMode::IMG2IMG || item->mode == QM::GenerationMode::TXT2IMG)
            {
                this->writeLog(wxString::Format("Diffusion started. Seed: %" PRId64 " Batch: %d %dx%dpx Cfg: %.1f Steps: %d\n",
                                                item->params.seed,
                                                item->params.batch_count,
                                                item->params.width,
                                                item->params.height,
                                                item->params.cfg_scale,
...
void MainWindowUI::HandleSDProgress(int step, int steps, float time, void *data)
{
    ...
    MainWindowUI::SendThreadEvent(eventHandler, sd_gui_utils::ThreadEvents::GENERATION_PROGRESS, myItem, wxString::Format("%d/%d", step, steps).ToStdString());
}
...
void MainWindowUI::UpdateJobInfoDetailsFromJobQueueList(QM::QueueItem *item)
{
    ...
    // check if the current selected item is equal with the item from the parameter, so only refresh if it bisible
    auto store = this->m_joblist->GetStore();
    auto row = this->m_joblist->GetSelectedRow();
    if (row == wxNOT_FOUND)
    {
    ...
    for (QM::QueueItemImage *img : item->images)
    {
        ...
        {
            auto resized = sd_gui_utils::cropResizeImage(img->pathname, 256, 256, wxColour(*wxBLACK), this->cfg->thumbs_path);
            img->id = index;
            ...
            bitmap->Bind(wxEVT_LEFT_DCLICK, [img](wxMouseEvent &event)
                         { wxLaunchDefaultApplication(std::filesystem::path(img->pathname).string()); });
            // rightclick
            bitmap->Bind(wxEVT_RIGHT_UP, [img, bitmap, item, this](wxMouseEvent &event)
                         {
                             ...
                             //menu->Enable(99,false);
                                ...
                                menu->AppendSeparator();
                                menu->Append(0, wxString::Format("Copy seed %" PRId64, item->params.seed + img->id));
                                menu->Append(1, "Copy prompts to text2img");
                                menu->Append(2, "Copy prompts to img2img");
                                //menu->Append(3, "Send the image to text2img");
                                menu->Append(3, "Send the image to img2img");

                                menu->Append(5, "Upscale");
                                   ...
                                   menu->Bind(wxEVT_COMMAND_MENU_SELECTED, [this,img,item](wxCommandEvent &evt){
                                        ...
                                        switch(id) {
                                                ...
                                                case 7: {
                                                    wxLaunchDefaultApplication(std::filesystem::path(img->pathname).parent_path().string());
                                                }break;
                                        default: {
                                            return;
                                        }break;
...
QM::QueueItem *MainWindowUI::handleSdImage(sd_image_t result, QM::QueueItem *itemPtr, wxEvtHandler *eventHandler)
{
    wxImage *img = new wxImage(result.width, result.height, result.data);

    if (!img->IsOk())
    {
        itemPtr->status_message = std::string("Invalid image from diffusion...");
    ...
    {
        itemPtr->status_message = wxString::Format("Failed to save image into %s", filename);
        MainWindowUI::SendThreadEvent(eventHandler, QM::QueueEvents::ITEM_FAILED, itemPtr);
        return itemPtr;
    }
    else
    {
        itemPtr->images.emplace_back(new QM::QueueItemImage({filename, QM::QueueItemImageType::GENERATED}));
...
{
    NewUpscalerCtxFunction new_upscaler_ctx = (NewUpscalerCtxFunction)this->sd_dll->GetSymbol("new_upscaler_ctx");
...
void MainWindowUI::GenerateTxt2img(wxEvtHandler *eventHandler, QM::QueueItem *myItem)
{
    ...
    if (!txt2img)
    {
        myItem->status_message = wxString(_("Failed to load img2img symbol from backend!"));
        MainWindowUI::SendThreadEvent(eventHandler, QM::QueueEvents::ITEM_FAILED, myItem);
        return;
    }
    FreeSdCtxFunction free_sd_ctx = (FreeSdCtxFunction)this->sd_dll->GetSymbol("free_sd_ctx");
    ...
    this->sd_set_progress_callback(MainWindowUI::HandleSDProgress, (void *)vparams);
    ...
    {
        myItem->status_message = wxString::Format("Model load failed: %s", myItem->model).ToStdString();
...
void MainWindowUI::GenerateImg2img(wxEvtHandler *eventHandler, QM::QueueItem *item)
{
    ...
    if (!img2img)
    {
        item->status_message = wxString(_("Failed to load img2img symbol from backend!"));
    ...
    if (!this->modelLoaded)
    {
        item->status_message = wxString::Format("Model load failed: %s", item->model).ToStdString();
        MainWindowUI::SendThreadEvent(eventHandler, QM::QueueEvents::ITEM_MODEL_FAILED, item);
        return;
    }
    ...
    {
        item->status_message = wxString::Format("Can not open initial image: %s", item->initial_image).ToStdString();
        MainWindowUI::SendThreadEvent(eventHandler, QM::QueueEvents::ITEM_FAILED, item);
        return;
    }
    ...
    int c = 0;
    int w, h;
    stbi_uc *input_image_buffer = stbi_load(item->initial_image.c_str(), &w, &h, &c, 3);
    ...
    sd_image_t *results = img2img(this->txt2img_sd_ctx,
                                  input_image,
                                  item->params.prompt.c_str(),
                                  item->params.negative_prompt.c_str(),
                                  item->params.clip_skip,
                                  item->params.cfg_scale,
                                  item->params.width,
                                  item->params.height,
                                  item->params.sample_method,
                                  item->params.sample_steps,
                                  item->params.strength,
                                  item->params.seed,
                                  item->params.batch_count);
...
void MainWindowUI::GenerateUpscale(wxEvtHandler *eventHandler, QM::QueueItem *item)
{
    ...
    UpscalerFunction upscale = (UpscalerFunction)this->sd_dll->GetSymbol("upscale");

    if (!upscale)
    {
        item->status_message = wxString(_("Failed to load upscale symbol from backend!"));
        MainWindowUI::SendThreadEvent(eventHandler, QM::QueueEvents::ITEM_FAILED, item);
        return;
    }
    FreeUpscalerFunction free_upscaler_ctx = (FreeUpscalerFunction)this->sd_dll->GetSymbol("free_upscaler_ctx");
    ...
    {
        MainWindowUI::SendThreadEvent(eventHandler, QM::QueueEvents::ITEM_MODEL_LOAD_START, item);
        this->upscaler_sd_ctx = this->LoadUpscaleModel(eventHandler, item);

        if (!this->upscaleModelLoaded || this->upscaler_sd_ctx == NULL)
        {
            item->status_message = wxString::Format("Model load failed: %s", item->params.esrgan_path).ToStdString();
            MainWindowUI::SendThreadEvent(eventHandler, QM::QueueEvents::ITEM_MODEL_FAILED, item);
            this->upscaleModelLoaded = false;
            return;
        }
    ...
    if (std::filesystem::exists(item->initial_image))
    {
        ...
        {
            item->status_message = wxString::Format("Failed to save image into %s", filename).ToStdString();
            MainWindowUI::SendThreadEvent(eventHandler, QM::QueueEvents::ITEM_FAILED, item);
        }
