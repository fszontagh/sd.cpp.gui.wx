...
set(CPACK_STRIP_FILES TRUE)
    ...
    if (SDGUI_SERVER OR EXISTS "${CMAKE_BINARY_DIR}/server/${EPREFIX}${PROJECT_BINARY_NAME}_server${CMAKE_EXECUTABLE_SUFFIX}")
...
if (SDGUI_LOCAL_DISABLE EQUAL OFF)
    ...
    if (SDGUI_CUDA OR EXISTS "${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}stable-diffusion_cuda${CMAKE_SHARED_LIBRARY_SUFFIX}")
    ...
    if (SDGUI_LLAMA_DISABLE EQUAL OFF)
        # llama cpp
        if (SDGUI_AVX OR EXISTS "${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_avx${CMAKE_SHARED_LIBRARY_SUFFIX}")
            install(FILES ${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_avx${CMAKE_SHARED_LIBRARY_SUFFIX}
            DESTINATION ${lib_INSTALL_PATH_NSIS}
            COMPONENT libllama_avx)
            list(APPEND CPACK_COMPONENTS_ALL "libllama_avx")
            set(CPACK_DEBIAN_LIBLLAMA_AVX_PACKAGE_NAME "libllama-avx-${SDCPP_VERSION}")
            set(CPACK_DEBIAN_LIBLLAMA_AVX_PACKAGE_RELEASE "${DISTRO_VERSION}")
        endif()

        if (SDGUI_AVX2 OR EXISTS "${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_avx2${CMAKE_SHARED_LIBRARY_SUFFIX}")
            install(FILES ${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_avx2${CMAKE_SHARED_LIBRARY_SUFFIX}
            DESTINATION ${lib_INSTALL_PATH_NSIS}
            COMPONENT libllama_avx2)
            list(APPEND CPACK_COMPONENTS_ALL "libllama_avx2")
            set(CPACK_DEBIAN_LIBLLAMA_AVX2_PACKAGE_NAME "libllama-avx2-${SDCPP_VERSION}")
            set(CPACK_DEBIAN_LIBLLAMA_AVX2_PACKAGE_RELEASE "${DISTRO_VERSION}")
        endif()

        if (SDGUI_AVX512 OR EXISTS "${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_avx512${CMAKE_SHARED_LIBRARY_SUFFIX}")
            install(FILES ${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_avx512${CMAKE_SHARED_LIBRARY_SUFFIX}
            DESTINATION ${lib_INSTALL_PATH_NSIS}
            COMPONENT libllama_avx512)
            list(APPEND CPACK_COMPONENTS_ALL "libllama_avx512")
            set(CPACK_DEBIAN_LIBLLAMA_AVX512_PACKAGE_NAME "libllama-avx512-${SDCPP_VERSION}")
            set(CPACK_DEBIAN_LIBLLAMA_AVX512_PACKAGE_RELEASE "${DISTRO_VERSION}")
        endif()

        if (SDGUI_CUDA OR EXISTS "${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}libllama_cuda${CMAKE_SHARED_LIBRARY_SUFFIX}")
            install(FILES ${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_cuda${CMAKE_SHARED_LIBRARY_SUFFIX}
            DESTINATION ${lib_INSTALL_PATH_NSIS}
            COMPONENT libllama_cuda)
            list(APPEND CPACK_COMPONENTS_ALL "libllama_cuda")
            set(CPACK_DEBIAN_LIBLLAMA_CUDA_PACKAGE_NAME "libllama-cuda-${SDCPP_VERSION}")
            set(CPACK_DEBIAN_LIBLLAMA_CUDA_PACKAGE_RELEASE "${DISTRO_VERSION}")
        endif()

        if (SDGUI_HIPBLAS OR EXISTS "${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}libllama_hipblas${CMAKE_SHARED_LIBRARY_SUFFIX}")
            install(FILES ${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_hipblas${CMAKE_SHARED_LIBRARY_SUFFIX}
            DESTINATION ${lib_INSTALL_PATH_NSIS}
            COMPONENT libllama_hipblas)
            list(APPEND CPACK_COMPONENTS_ALL "libllama_hipblas")
            set(CPACK_DEBIAN_LIBLLAMA_HIPBLAS_PACKAGE_NAME "libllama-hipblas-${SDCPP_VERSION}")
            set(CPACK_DEBIAN_LIBLLAMA_HIPBLAS_PACKAGE_RELEASE "${DISTRO_VERSION}")
        endif()

        if (SDGUI_VULKAN OR EXISTS "${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}libllama_vulkan${CMAKE_SHARED_LIBRARY_SUFFIX}")
            install(FILES ${CMAKE_BINARY_DIR}/${EPREFIX}${CMAKE_SHARED_LIBRARY_PREFIX}llama_vulkan${CMAKE_SHARED_LIBRARY_SUFFIX}
            DESTINATION ${lib_INSTALL_PATH_NSIS}
            COMPONENT libllama_vulkan)
            list(APPEND CPACK_COMPONENTS_ALL "libllama_vulkan")
            set(CPACK_DEBIAN_LIBLLAMA_VULKAN_PACKAGE_NAME "libllama-vulkan-${SDCPP_VERSION}")
            set(CPACK_DEBIAN_LIBLLAMA_VULKAN_PACKAGE_RELEASE "${DISTRO_VERSION}")
        endif()
    endif()
endif()
