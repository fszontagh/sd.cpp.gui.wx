...
namespace QM
{
    ...
    struct QueueItemImage
    {
        ...
        QueueItemImage(const QueueItemImage &other)
            : pathname(other.pathname), type(other.type) {}
        QueueItemImage(std::string pathname_, QM::QueueItemImageType type_ = QM::QueueItemImageType::GENERATED)
    ...
    inline void to_json(json &j, const QueueItemImage *item)
    {
        if (item)
        {
            j = json{{"pathname", sd_gui_utils::UnicodeToUTF8(item->pathname)}, {"type", item->type}};
        }
        else
        {
            j = nullptr;
    ...
    inline void from_json(const json &j, QueueItemImage *&item)
    {
        if (!j.is_null())
        {
    ...
    struct QueueItem
    {
        ...
        std::vector<QM::QueueItemImage *> images = std::vector<QM::QueueItemImage *>();
