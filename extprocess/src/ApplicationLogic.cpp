...
void ApplicationLogic::Txt2Img() {
    try {
        ...
        gen_params.sample_params.guidance.txt_cfg        = this->currentItem->params.cfg_scale;
        gen_params.sample_params.guidance.img_cfg        = this->currentItem->params.guidance;
        gen_params.sample_params.guidance.distilled_guidance = 0.0f;
        gen_params.sample_params.guidance.slg.layers     = this->currentItem->params.skip_layers.data();
        gen_params.sample_params.guidance.slg.layer_count = this->currentItem->params.skip_layers.size();
        gen_params.sample_params.guidance.slg.layer_start = this->currentItem->params.skip_layer_start;
        gen_params.sample_params.guidance.slg.layer_end  = this->currentItem->params.skip_layer_end;
        gen_params.sample_params.guidance.slg.scale      = this->currentItem->params.slg_scale;
    ...
    if (this->currentItem->mode == SDMode::UPSCALE) {
        ...
        if (this->lastItem != nullptr && this->lastItem->mode == SDMode::UPSCALE) {
            ...
            if (this->lastItem->params.esrgan_path != this->currentItem->params.esrgan_path) {
                ...
                this->upscale_ctx = this->newUpscalerCtxPtr(this->currentItem->params.esrgan_path.c_str(), false, false, this->currentItem->params.n_threads);
